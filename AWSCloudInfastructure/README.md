📂 AWSCloudInfastructure / README.md
...
The Workflow:
Trigger: A customer sends a WhatsApp message via the Twilio API.

Ingress: AWS API Gateway receives the webhook POST request and validates the payload.

Compute: AWS Lambda is triggered. It executes a Python-based logic engine to parse the message and determine the intent.

Integration: Data is pushed to the Nexum Group intake database/CRM.

Egress: An automated response is sent back through the Twilio Sandbox.
...
[ ] Asynchronous Processing: Introduce AWS SQS (Simple Queue Service) to decouple message reception from database writes, ensuring 100% message delivery reliability.
...
Note: The API endpoints and Secrets are managed via the AWS Management Console and are not exposed in this repository for security reasons.
